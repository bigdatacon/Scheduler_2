...
# Функция для обработки движения мыши при перетаскивании
def on_motion(event):
    global active_rect, dragging
    if dragging and active_rect is not None:
        if event.xdata is None or event.ydata is None:
            return
        op, ax, text, color = rect_dict[active_rect]
        new_start = int(event.xdata)
        new_finish = new_start + (op['Finish'] - op['Start'])
        active_rect.set_x(new_start)
        active_rect.set_width(new_finish - new_start)
        text.set_x(new_start + (new_finish - new_start) / 2)

        # Перетаскивание по оси Y
        if ax == ax1:
            new_machine = int(round(event.ydata))
            op['Machine'] = new_machine
            new_y = [i for i, key in enumerate(sorted(range(1, max(op['Machine'] for op in js_operations) + 3), reverse=True)) if key == new_machine][0]
        else:
            new_job = int(round(event.ydata))
            op['Job'] = new_job
            new_y = [i for i, key in enumerate(sorted(range(1, max(op['Job'] for op in js_operations) + 3), reverse=True)) if key == new_job][0]

        active_rect.set_y(new_y - 0.4)
        text.set_y(new_y)
        start_box.set_val(str(new_start))
        finish_box.set_val(str(new_finish))
        job_box.set_val(str(op['Job']))
        machine_box.set_val(str(op['Machine']))
        fig.canvas.draw_idle()
    ...
    if active_rect is not None:
        ...
        for rect, (related_op, related_ax, related_text, related_color) in rect_dict.items():
            if related_op == op and rect != active_rect:
                ...
                if related_ax == ax1:
                    related_y = [i for i, key in enumerate(sorted(range(1, max(op['Machine'] for op in js_operations) + 3), reverse=True)) if key == new_machine][0]
                    rect.set_y(related_y - 0.4)
                    related_text.set_y(related_y)
                elif related_ax == ax2:
                    related_y = [i for i, key in enumerate(sorted(range(1, max(op['Job'] for op in js_operations) + 3), reverse=True)) if key == new_job][0]
                    rect.set_y(related_y - 0.4)
                    related_text.set_y(related_y)
